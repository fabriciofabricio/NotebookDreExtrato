...
async function salvarCategorizacao() {
  if (linhasNaoCategorizada.length === 0) {
  ...
  try {
    ...
    if (alteracoes > 0) {
      // MUDANÇA: Salvar no arquivo do servidor em vez de apenas no localStorage
      const resultado = await salvarCategoriasNoArquivo(mapeamentoCategorias, categoriasPersonalizadas);
      
      // Regenerar o DRE
      gerarDRE();
      
      if (resultado.success) {
        alert(`${alteracoes} item(s) categorizado(s) com sucesso! As categorias foram salvas no arquivo categorias.js.`);
      } else {
        alert(`${alteracoes} item(s) categorizado(s), mas houve um problema ao salvar no arquivo: ${resultado.message}`);
      }
    } else {
      alert('Nenhuma alteração foi feita. Selecione categorias para os itens.');
    }
  } catch (erro) {
    console.error('Erro ao salvar categorização:', erro);
    alert('Erro ao salvar categorização: ' + erro.message);
  } finally {
