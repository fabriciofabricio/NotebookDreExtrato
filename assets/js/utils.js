...
// Salvar categorias diretamente no arquivo categorias.js no servidor
async function salvarCategoriasNoArquivo(mapeamentoCategorias, categoriasPersonalizadas) {
try {
  // URL do endpoint no servidor
  const url = 'http://localhost:3000/api/salvar-categorias';
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      estruturaDRE // Isso assume que estruturaDRE é uma variável global
    }),
  });
  ...
  console.error('Erro ao salvar categorias no arquivo:', erro);
  ...
  return {
    success: false,
    message: 'Não foi possível salvar no arquivo. Dados salvos apenas no localStorage como fallback.',
    error: erro.message
  };
}
}